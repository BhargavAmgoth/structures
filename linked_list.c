...
struct node *creat_linked_list(struct node *start);
...
struct node *add_link_pos(struct node *start, int pos, int data, int item);
...
int main()
{
	...
	scanf("%d", &choice);
		...
		{
			case 1:
				start = creatr_linked_list(start);
				break;
			case 2: 
...
struct node *add_link_before(struct node *start, int data, int temp)
{
	struct node *temp, *p;
	if(start == NULL)
	{
...
struct node *add_link_after(struct node *start, int data, int temp)
{
	struct node *temp, *p;
	if(start == NULL)
	{
...
struct node *create_linked_list(struct node *start)
{
	...
	{
		prinf("Enter  the info for %d node\n", i);
		scanf("%d", &data);
...
struct node *search_link(struct node *start, int data)
{
	struct node *temp=start;
	int pos=1;
	while(temp->link != NULL)
